...
class BetSettlementAcceptanceTest {
  ...
  static DockerComposeContainer<?> compose =
      new DockerComposeContainer<>(new File("src/test/resources/docker-compose-test.yml"))
          ...
          .withExposedService(
              REDIS_SERVICE,
              REDIS_PORT,
              Wait.forHealthcheck().withStartupTimeout(Duration.ofSeconds(30)));
  ...
  void shouldSettleBetsEndToEndWhenEventOutcomePublished() {
    EventOutcomeRequest request =
        new EventOutcomeRequest()
            .eventId("event-acceptance-1")
            .eventName("Acceptance Test Match")
            .eventWinnerId("team-alpha");

    ResponseEntity<String> response =
        restTemplate.postForEntity("/api/v1/event-outcomes", request, String.class);
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              Optional<Bet> winnerOpt = betRepository.findById(betWinnerId);
              assertThat(winnerOpt).isPresent();
              assertThat(winnerOpt.get().getStatus()).isEqualTo(BetStatus.WON);
              assertThat(winnerOpt.get().getSettledAt()).isNotNull();
            });
    ...
    await()
        ...
        .untilAsserted(
            () -> {
              Optional<Bet> loserOpt = betRepository.findById(betLoserId);
              assertThat(loserOpt).isPresent();
              assertThat(loserOpt.get().getStatus()).isEqualTo(BetStatus.LOST);
              assertThat(loserOpt.get().getSettledAt()).isNotNull();
            });
  }
