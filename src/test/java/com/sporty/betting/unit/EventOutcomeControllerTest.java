...
import com.sporty.betting.config.JacksonConfig;
...
class EventOutcomeControllerTest {
  ...
  @MockitoBean private EventOutcomeService eventOutcomeService;
  ...
  void shouldReturn400WhenEventIdMissing() throws Exception {
    EventOutcomeRequest request =
        new EventOutcomeRequest().eventName("Champions League Final").eventWinnerId("team-a");
  ...
  void shouldReturn400WhenEmptyBody() throws Exception {
    mockMvc
        .perform(
            post("/api/v1/event-outcomes").contentType(MediaType.APPLICATION_JSON).content("{}"))
        .andExpect(status().isBadRequest());
  }
}
